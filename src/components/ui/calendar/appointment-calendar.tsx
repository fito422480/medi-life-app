...
export function AppointmentCalendar({ doctorId, patientId }: AppointmentCalendarProps) {
  const { user } = useAuth();
  const { appointments, isLoading, error } = useAppointments(doctorId, patientId);
  ...
  const renderAppointment = (date: Date, hour: number, minute: number) => {
    ...
    return (
      <div 
        className={`p-1 rounded border ${statusColors[appointment.status]} hover:opacity-80 cursor-pointer`}
        onClick={() => {
      ...
      >
        ...
        <p className="text-xs truncate">{appointment.reason.substring(0, 20)}</p>
      </div>
    );
  };
  ...
  if (isLoading) {
    return <div className="flex justify-center p-10">Cargando calendario...</div>;
  }

  if (error) {
    return <div className="text-red-500 p-10">Error al cargar el calendario: {error}</div>;
  }
    ...
    <div className="container mx-auto p-4">
      <Card className="shadow-lg border-0">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle className="text-2xl font-bold">Calendario de Citas</CardTitle>
            <CardDescription>
              {format(currentWeek[0], "'Semana del' d 'de' MMMM", { locale: es })} - 
        ...
        <CardContent>
                ...
                {currentWeek.map((day, index) => (
                  <div 
                    ...
                    className={`h-12 bg-muted rounded-md flex flex-col items-center justify-center font-medium p-1
                      ${isToday(day) ? 'bg-primary/20 border border-primary/50' : ''}
                    `}
                  >
                ...
                <div className="space-y-1">
                  {HOURS.flatMap(hour => {
                    return [0, 30].map(minute => (
                      <div key={`${hour}-${minute}`} className="grid grid-cols-6 gap-1 min-h-[60px]">
                        ...
                        {currentWeek.map((day, dayIndex) => {
                          ...
                          const isAvailable = isTimeSlotAvailable(day, hour, minute);
                          ...
                          return (
                            <div 
                              ...
                              className={`rounded-md p-1 border ${
                                hasAppointment ? '' : 
                                isAvailable ? 'border-dashed border-gray-200 hover:bg-green-50 hover:border-green-300 cursor-pointer' : 
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            <Badge variant="outline" className="bg-yellow-100 border-yellow-300">Pendiente</Badge>
            <Badge variant="outline" className="bg-green-100 border-green-300">Confirmada</Badge>
            <Badge variant="outline" className="bg-blue-100 border-blue-300">Completada</Badge>
            <Badge variant="outline" className="bg-red-100 border-red-300">Cancelada</Badge>
            <Badge variant="outline" className="bg-gray-100 border-gray-300">No asistió</Badge>
          </div>
        </CardContent>
      </Card>
